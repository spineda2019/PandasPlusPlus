...
class Dataframe {
 public:
     ...
     * @param col: String specifying which column to get mean from. Only works
     ...
     * @param omit_nan: Ignores NaNs in all calculations if true, treats them as
     * 0 if false
     * @return The column's mean on success, returns NaN on failure
     */
        ...
        if (!omit_nan) {
            ...
            std::transform(
                ...
                [this, &col_index](T &x) {
                    T val{};
                    // catch NaN
                    if (!std::isnan((this->data_)[static_cast<std::size_t>(x)]
                                                 [col_index])) {
     ...
     * @brief Get the mean of a specific row of a dataframe
     * @param index: row index to calculate mean
     * @param omit_nan: Ignores NaNs on all calculations if true, treats them as
     * 0 if false
     * @return The row's mean on success, returns NaN on failure
     */
    ...
    /**
     * @brief Stack this's dataframe onto a given dataframe to concatenate the
     * two
