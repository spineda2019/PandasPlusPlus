/*
 *  DSP.hpp
 *  Helper functions for common digital signal processing tasks
 *
...
//                                 {
//     std::vector<std::vector<std::complex<T>>> k(signal.size());  // Column
//     vec
//
//     // init k w/ 0,0
...
//     // e dot signal
//     std::transform(
//         std::execution::par_unseq, k.begin(), k.end(),
//         domain_to_fill.begin(),
//         [&signal, &N](auto &x) {
//             const std::complex<T> dot =
//                 std::inner_product(x.begin(), x.end(), signal.begin(),
//                                    std::complex<T>(T(0.0), T(0.0)));
//             return dot / (std::complex<T>(N) / std::complex<T>(T(2.0),
//             T(0.0)));
//         });
// }
//
// /**
//  * @brief Extract N frequencies from a signal from highest amplitude to
//  lowest
//  *
...
//  * @param[in] n: desired number of amplitudes to get (optional, defaults to
//  3)
//  *
//  */
// template <class T>
...
//  */
// template <class T>
// inline void ExtractNPhases(const std::vector<T> &signal, std::vector<T>
// &dest,
//                            const std::size_t n) {
//     if (dest.size()) {
//         std::cout << "Error: Non-empty destination vector received"
